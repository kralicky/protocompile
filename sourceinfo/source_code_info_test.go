...
func TestSourceCodeInfo(t *testing.T) {
	...
	importedFd := fds.SortedFiles[0].FindImportByPath("desc_test_options.proto")
	require.NotNil(t, importedFd)

	fdset := prototest.LoadDescriptorSet(t, "../internal/testdata/source_info.protoset", linker.ResolverFromFile(fds.SortedFiles[0]))
	actualFdset := &descriptorpb.FileDescriptorSet{
		File: []*descriptorpb.FileDescriptorProto{
			protoutil.ProtoFromFileDescriptor(importedFd),
			protoutil.ProtoFromFileDescriptor(fds.SortedFiles[0]),
			protoutil.ProtoFromFileDescriptor(fds.SortedFiles[1]),
		},
	}
...
func TestSourceCodeInfoOptions(t *testing.T) {
	...
	generateSourceInfoText := func(filename string, mode protocompile.SourceInfoMode) string {
		...
		file, err := linker.NewFileRecursive(fds.SortedFiles[0])
		require.NoError(t, err)
